...
namespace SuperMapSDK
{
	namespace UnrealEngine
	{
		...
		{
			bool FTexUtility::IsHDR(EPixelFormat Format)
			{
				switch (Format)
				{
			...
			bool FTexUtility::IsAlphaless(EPixelFormat PixelFormat)
			{
				switch (PixelFormat)
				{
				case PF_ATC_RGB:
			...
			bool FTexUtility::IsCubemap(const UTexture* Texture)
			{
				return Texture->IsA<UTextureCube>() || Texture->IsA<UTextureRenderTargetCube>();
			}

			float FTexUtility::GetCubeFaceRotation(ECubeFace CubeFace)
			{
				switch (CubeFace)
				{
			...
			TextureFilter FTexUtility::GetDefaultFilter(TextureGroup LODGroup)
			{
			...
			int32 FTexUtility::GetMipBias(const UTexture* Texture)
			{
			...
			FIntPoint FTexUtility::GetInGameSize(const UTexture* Texture)
			{
			...
			TTuple<TextureAddress, TextureAddress> FTexUtility::GetAddressXY(const UTexture* Texture)
			{
			...
			UTexture2D* FTexUtility::CreateTransientTexture(const void* RawData, int64 ByteLength, const FIntPoint& Size, EPixelFormat Format, bool bSRGB)
			{
			...
			UTextureRenderTarget2D* FTexUtility::CreateRenderTarget(const FIntPoint& Size, bool bHDR)
			{
			...
			bool FTexUtility::DrawTexture(UTextureRenderTarget2D* OutTarget, const UTexture2D* InSource, const FVector2D& InPosition, const FVector2D& InSize, const FMatrix& InTransform)
			{
			...
			bool FTexUtility::RotateTexture(UTextureRenderTarget2D* OutTarget, const UTexture2D* InSource, const FVector2D& InPosition, const FVector2D& InSize, float InDegrees)
			{
			...
			UTexture2D* FTexUtility::CreateTextureFromCubeFace(const UTextureCube* TextureCube, ECubeFace CubeFace)
			{
			...
			UTexture2D* FTexUtility::CreateTextureFromCubeFace(const UTextureRenderTargetCube* RenderTargetCube, ECubeFace CubeFace)
			{
			...
			bool FTexUtility::ReadPixels(const UTextureRenderTarget2D* InRenderTarget, TArray<FColor>& OutPixels/*, EGLTFJsonHDREncoding Encoding*/)
			{
			...
			void FTexUtility::EncodeRGBM(const TArray<FLinearColor>& InPixels, TArray<FColor>& OutPixels, float MaxRange)
			{
			...
			void FTexUtility::EncodeRGBE(const TArray<FLinearColor>& InPixels, TArray<FColor>& OutPixels)
			{
				OutPixels.AddUninitialized(InPixels.Num());
			...
			bool FTexUtility::LoadPlatformData(UTexture2D* Texture)
			{
			...
			bool FTexUtility::LoadPlatformData(UTextureCube* TextureCube)
			{
			...
			void FTexUtility::FlipGreenChannel(TArray<FColor>& Pixels)
			{
