...
class FSuperMapMeshSceneProxy final : public FPrimitiveSceneProxy
{
public:
	FSuperMapMeshSceneProxy(const USuperMap_MeshComponent* InComponent, FSMMesh& smMesh, const TArray<FSuperMapMaterialInterfaceCollection>& arrMaterialCollection);
	virtual ~FSuperMapMeshSceneProxy();
	...
	Gamevoid UpdateCustomVertexAttribute0_RenderThread(const FSMMesh& smMesh);
	Gamevoid UpdateVertexColor_RenderThread(const FSMMesh & smMesh);
	Gamevoid UpdateVertex_RenderThread(const FSMMesh& smMesh);
	...
	static Gamevoid SetSceneUseMaterialFlag_RenderThread(UseMaterialFlag enUseMaterialFlag);

	Gamevoid AddMaterialInterface(UMaterialInterface* materialInterface);
	Gamevoid RemoveMaterialInterface(UMaterialInterface* materialInterface);

private:
	Gamebool IsInsideFrustum(const FSceneView* View) const;
	FMaterialRenderProxy* GetMaterialForSection(const FSuperMapRenderSection& section, Gameint viewIndex) const;
	...
	TArray<UMaterialInterface*> m_arrMaterialInterface;
	
#if RHI_RAYTRACING
	FRayTracingGeometry m_RayTracingGeometry;
#endif

	static UseMaterialFlag m_enSceneUseMaterialFlag;
};
